...
public class NodeCommunicatorTer extends Thread {

    private ServerSocket serverSocket;
    private Set<NodeCommunicatorThreadTer> nodeCommunicatorThreads = new HashSet<NodeCommunicatorThreadTer>();

    public NodeCommunicatorTer(String port) throws NumberFormatException, IOException {
        serverSocket = new ServerSocket(Integer.valueOf(port) + 20, 500);
    }

    public void run() {
        try {
            while (true) {
                NodeCommunicatorThreadTer nodeCommunicatorThread = new NodeCommunicatorThreadTer(serverSocket.accept(),
                        this);
    ...
    public Set<NodeCommunicatorThreadTer> getnodeCommunicatorThread() {
