...
public class QBFTConsensusMessageHandler extends Thread {
    ...
    public QBFTMessagePool qbftMessagePool;
    ...
    public QBFTConsensusRoundChangeHandler qbftConsensusRoundChangeHandler;
                            ...
                            if (messageType.equals("PRE-PREPARE")) {
                                ...
                                if (this.blockchain.isValidBlock(inblock)) {
                                    if (!this.blockPool.blockIds.contains(inblock.getBlockNumber())) {
                                        ...
                                        Message message = this.qbftMessagePool.message("PREPARE", inblock, this.wallet,
                                    ...
                                    this.qbftConsensusRoundChangeHandler.task();
                                }
                                ...
                                if (!this.qbftMessagePool.existingPrepareMessage(inmessage)
                                        && this.qbftMessagePool.isValidMessage(inmessage)
                                        && this.validator.isValidValidator(inmessage.getMessageSender())
                                        && !this.synchronizer.prepareconsensusReached.get(blockhash)) {
                                    this.qbftMessagePool.addMessage(inmessage);
                                    ...
                                    if (this.qbftMessagePool.preparePool.get(inmessage.getBlockHash())
                                            .size() >= this.minapprovals
                                            || this.qbftMessagePool.preparePool.get(inmessage.getBlockHash())
                                                    .size() >= this.minapprovalsalt) {
                                        if (NodeProperty.getnodeBehavior() == 0) {

                                            this.synchronizer.prepareconsensusReached.put(blockhash, true);
                                            Message message = this.qbftMessagePool.message("COMMIT", inmessage,
                                ...
                                Block inblock = preparegson.fromJson(jsonObject.getString("blockdata"), blockObject);
                                if (!this.qbftMessagePool.existingCommitMessage(commitmessage)
                                        && this.qbftMessagePool.isValidMessage(commitmessage)
                                        && this.validator.isValidValidator(commitmessage.getMessageSender())
                                        && !this.synchronizer.commitconsensusReached.get(blockhash)) {

                                    this.qbftMessagePool.addMessage(commitmessage);
                                }
                                try {
                                ...
                                if (this.qbftMessagePool.commitPool.get(commitmessage.getBlockHash())
                                        .size() >= this.minapprovals
                                        || this.qbftMessagePool.commitPool.get(commitmessage.getBlockHash())
                                                .size() >= this.minapprovalsalt) {
                                    // Reschedule
                                    // this.qbftConsensusRoundChangeHandler.rescheduleTimer();
                                    this.synchronizer.commitconsensusReached.put(blockhash, true);
                                    this.blockchain.addUpdatedBlock(inblock, this.blockPool,
                                            this.qbftMessagePool);
                                ...
                                this.simulator_result.prepareCounter
                                        .put(new Timestamp(System.currentTimeMillis()), this.qbftMessagePool
                                ...
                                Message roundchangeMessage = gson.fromJson(jsonObject.getString("data"),
                                        messageObject);
                                if (!this.qbftMessagePool.existingRoundChangeMessage(roundchangeMessage)
                                        && this.qbftMessagePool.isValidMessage(roundchangeMessage)
                                    ...
                                    this.qbftMessagePool.addMessage(roundchangeMessage);
                                    ...
                                    if (this.qbftMessagePool.roundChangePool.get(roundchangeMessage.getBlockHash())
                                            .size() >= this.minapprovals
                                            || this.qbftMessagePool.roundChangePool
    ...
    public QBFTConsensusMessageHandler(BlockingQueue<JSONObject> inMessageBlockingQueue, Blockchain blockChain,
            TransactionPool transactionPool,
            BlockPool blockPool, Wallet wallet, Validator validator, NonValidator nonValidator,
            NodeCommunicator nodeCommunicator, String currentuser, QBFTMessagePool qbftMessagePool,
        ...
        this.qbftMessagePool = qbftMessagePool;
        // (2N/3)+1

        float totalfloat = (float) totalValidators;
        float minapprovalsfloat = (totalfloat * 2.0f / 3.0f);
        int minapprovalint = (int) minapprovalsfloat;
        this.minapprovals = minapprovalint;
        float minapprovalsfloatalt = (totalfloat * 1.0f / 3.0f);
        ...
        this.qbftConsensusRoundChangeHandler = qbftConsensusRoundChangeHandler;
    }
}