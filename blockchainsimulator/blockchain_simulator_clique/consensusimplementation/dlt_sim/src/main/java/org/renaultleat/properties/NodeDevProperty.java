package org.renaultleat.properties;

public class NodeDevProperty {

    public static final int nodeid = 0;
    public static final int nodeid_sec = 1;
    public static final int totalkeys = 20;
    // Set value for the testing
    public static final int totalnodes = 2;
    public static final String users = "node1,node2,node3,node4,node5,node6,node7,node8";
    public static final String ip = "localhost";
    public static final String ports = "7080,7081,7082,7083,7084,7085,7086,7087";
    public static final int blocksize = 30;
    public static final int minapprovals = (totalnodes / 2) + 1;
    public static final String deployed = "01 JAN2022";

}
