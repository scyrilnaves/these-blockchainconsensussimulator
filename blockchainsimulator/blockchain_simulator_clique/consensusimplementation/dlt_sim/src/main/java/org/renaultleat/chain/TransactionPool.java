...
public class TransactionPool {
    ...
    public synchronized boolean addTransaction(Transaction transaction) {
        ...
        if (!this.transactionStorage.containsKey(this.roundcounter.get())) {
            CopyOnWriteArrayList<Transaction> transactions = new CopyOnWriteArrayList<Transaction>();
            this.transactionStorage.put(this.roundcounter.get(), transactions);
        }
        if (!this.transactionRoundStatus.containsKey(this.roundcounter.get())) {
            this.transactionRoundStatus.put(this.roundcounter.get(), false);
        }
        // Check if the transaction can be added to the round
        // Or we need to synchronise the current blockchain
        if (!this.transactionRoundStatus.get(this.roundcounter.get())) {
            this.transactionStorage.get(this.roundcounter.get()).add(transaction);
        } /*
           * else {
           ...
           */
        // THRESHOLD REACHED
        if (this.transactionStorage.get(this.roundcounter.get()).size() >= this.transactionThreshold) {

            this.transactionRoundStatus.put(this.roundcounter.get(), true);
            this.roundcounter.incrementAndGet();
            // System.out.println("called increment" + this.roundcounter.get());
            return true;
        } else {
            return false;
        }
