...
for noofnode in "${noofnodes[@]}"; do
  for noofvalidator in "${noofvalidators[@]}"; do
    for nwtype in "${networktype[@]}"; do
      for blperiod in "${blockperiod[@]}"; do
        for nwlaten in "${latency[@]}"; do
          for blsize in "${blocksize[@]}"; do
            ...
            ./mainexecutetest.sh $noofnode $nwtype $blsize $nwlaten $nodebehaviour $totaltxs $noofvalidator $isvalidator $blperiod

            testtype="NoNode:"$noofnode";NwType:"$nwtype";BlockSize:"$blsize";NwLatency:"$nwlaten";NodeBehaviour:"$nodebehaviour";TotalTxs:"$totaltxs";Consensus:"$consensus";TotalValidators:"$noofvalidator";IsValidator:"$isvalidator";BlockPeriod:"$blperiod
