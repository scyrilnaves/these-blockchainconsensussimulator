...
public class NodeCommunicatorQuar extends Thread {

    private ServerSocket serverSocket;
    private Set<NodeCommunicatorThreadQuar> nodeCommunicatorThreads = new HashSet<NodeCommunicatorThreadQuar>();

    public NodeCommunicatorQuar(String port) throws NumberFormatException, IOException {
        serverSocket = new ServerSocket(Integer.valueOf(port) + 30, 500);
    }
        ...
        try {
            while (true) {
                if (nodeCommunicatorThreads.size() < (NodeProperty.gettotalNodes() - 1)) {
                    NodeCommunicatorThreadQuar nodeCommunicatorThread = new NodeCommunicatorThreadQuar(
                            serverSocket.accept(),
                            this);
    ...
    public Set<NodeCommunicatorThreadQuar> getnodeCommunicatorThread() {
