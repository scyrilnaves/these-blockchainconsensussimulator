...
public class NodeCommunicatorSix extends Thread {

    private ServerSocket serverSocket;
    private Set<NodeCommunicatorThreadSix> nodeCommunicatorThreads = new HashSet<NodeCommunicatorThreadSix>();

    public NodeCommunicatorSix(String port) throws NumberFormatException, IOException {
        serverSocket = new ServerSocket(Integer.valueOf(port) + 50, 500);
    }
        ...
        try {
            while (true) {
                if (nodeCommunicatorThreads.size() < (NodeProperty.gettotalNodes() - 1)) {
                    NodeCommunicatorThreadSix nodeCommunicatorThread = new NodeCommunicatorThreadSix(
                            serverSocket.accept(),
                            this);
    ...
    public Set<NodeCommunicatorThreadSix> getnodeCommunicatorThread() {
